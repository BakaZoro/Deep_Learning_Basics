...
f.write("Using l2 regualirisation with a penalty value of 0.01 \n")
...
for i in range(1): #No of times the entire model is run
	...
	for train_index, test_index in kf.split(iris_data):
		...
		for j in range(innerloop): #No of times the weight is initialised for each train and test dataset
			...
			model = Sequential()
			model.add(Dense(4, input_shape=(4,),kernel_regularizer=regularizers.l2(0.01),activation='linear',use_bias=True,kernel_initializer='glorot_uniform'))
			model.add(Dense(5,kernel_regularizer=regularizers.l2(0.01),activation='sigmoid',use_bias=True,kernel_initializer='glorot_uniform')) 
			model.add(Dense(3,kernel_regularizer=regularizers.l2(0.01),activation='softmax',use_bias=True,kernel_initializer='glorot_uniform')) 
